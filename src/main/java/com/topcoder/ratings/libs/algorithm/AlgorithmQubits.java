...
import com.topcoder.ratings.libs.model.QubitsRatingData;
...
public class AlgorithmQubits implements Algorithm {
    ...
    public void runRatings() {
        logger.info("Starting rating run for " + data.length + " entries");
        ...
        //debug output
        for(int i = 0; i < data.length; i++) {
            logger.debug("CDR: " + data[i].getCoderID() + ":" + data[i].getRating() + "," + data[i].getVolatility());
        }
    }
